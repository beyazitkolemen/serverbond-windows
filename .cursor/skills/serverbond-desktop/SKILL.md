...
`src-tauri` ince kalır. İş kuralı `crates/serverbond-core::Manager` içindedir; komutlar `blocking` + `contain()` ile çalışır.

## IPC

Komutlar `src-tauri/src/main.rs` içinde. Arayüz `src/api.ts` → `call()`. Okuma (`snapshot`, `read_log`, `read_project_log`, `read_project_worker_log`, `read_project_schedule_log`, `list_project_schedule`, `list_failed_jobs`, `list_project_releases`, `project_git_status`, `discover_projects`, …) 10 sn UI zaman aşımına girebilir; yazma/kurulum zaman aşımıyla yeniden başlatılmaz. `database` `create|backup|restore`; `https_trust` `trust|untrust`; `import_projects` yolları alır. Kuyruk: `save_project_jobs`, `start|stop|restart_project_worker`, `start|stop|restart_project_schedule`, `retry_failed_jobs`, `flush_failed_jobs`. Sürüm: `save_project_release`, `deploy_project` (10 dk, meşgul kilidi). `postgres` `install|repair|start|stop|credentials|password`. `redis` `install|repair|start|stop`. `github` `save|forget|import`. `read_project_env` / `save_project_env`. `read_project_log` kaynağı `php`, `schedule` veya `worker:<uuid>`.

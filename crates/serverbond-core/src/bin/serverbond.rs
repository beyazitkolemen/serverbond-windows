...
use serverbond_core::{model::Settings, Manager};
...
fn api_command(manager: Manager, args: &[String]) -> Result<()> {
    ...
    match args.first().map(String::as_str).unwrap_or("status") {
        ...
        "routes" => {
            for (method, path, description) in serverbond_core::api::routes() {
...
fn main() -> Result<()> {
    ...
    match args[0].as_str() {
        ...
        "redis" => match args.get(1).map(String::as_str).unwrap_or("status") {
            "install" => manager.install_redis()?,
            "repair" => manager.repair_redis()?,
            "start" => manager.start_redis()?,
            "stop" => manager.stop_redis()?,
            "status" => println!(
                "{}",
                serde_json::to_string_pretty(&manager.snapshot()?.redis)?
            ),
            _ => bail!("Kullanım: redis install|start|stop|repair|status"),
        },
...
fn smoke(manager: &Manager) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        manager.mysql_query(&format!("CREATE TABLE `{database}`.checks (id INT PRIMARY KEY, value VARCHAR(30)); INSERT INTO `{database}`.checks VALUES (1, 'serverbond-ok')"))?;
        let value =
            manager.mysql_query(&format!("SELECT value FROM `{database}`.checks WHERE id=1"))?;
        if value != "serverbond-ok" {
            bail!("MySQL veri doğrulaması başarısız.");
        }
        let backup = manager.backup_database(&name)?;
        let sql = std::fs::read_to_string(&backup)?;
        if !sql.contains("serverbond-ok") {

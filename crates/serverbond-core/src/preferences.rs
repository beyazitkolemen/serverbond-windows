...
impl Manager {
    pub(crate) fn validate_php_profile(&self, version: &str, settings: &PhpSettings) -> Result<()> {
        ...
        cmd.arg("-c")
            ...
            .args(["-r", &script])
            .env("SERVERBOND_PHP_EXT", root.join("ext"))
            .env("PHP_INI_SCAN_DIR", "");
        let output = ManagedChild::output(cmd, Duration::from_secs(20))?;
        if !output.status.success()
            || !output.stderr.is_empty()
            || String::from_utf8_lossy(&output.stdout).trim() != "SERVERBOND_OK"
        {
            bail!(
...
mod tests {
    ...
    fn php_mail_reaches_the_catcher_only_while_the_relay_is_on() {
        ...
        assert!(relayed.contains("sendmail_from=serverbond@localhost"));

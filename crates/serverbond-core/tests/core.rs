...
fn catalog_only_contains_pinned_https_packages() {
    ...
    for package in packages.into_iter().chain(serverbond_core::model::tools()) {
...
fn saved_php_selection_controls_paths_and_package_status() {
    let home = tempfile::tempdir().unwrap();
    let config = serverbond_core::model::Config {
...
fn laravel12_requires_php_8_2_or_newer_by_series() {
    use serverbond_core::model::{php_meets, php_supports_laravel12};
...
fn php_catalog_has_one_pinned_build_per_supported_series() {
    let versions = serverbond_core::model::php_versions();
...
fn optional_service_exposes_repair_when_install_is_broken() {
    let home = tempfile::tempdir().unwrap();
    let manager = Manager::new(home.path().into()).unwrap();
    let package = serverbond_core::model::tool_package("mailpit").unwrap();
...
fn archive_fallback_is_only_for_official_php_release_urls() {
    ...
    assert!(serverbond_core::install::archive_fallback(&php)
        .unwrap()
        .contains("/archives/php-8.4.25"));
    php.url = "https://example.com/php.zip".into();
    assert!(serverbond_core::install::archive_fallback(&php).is_none());
    php.url = "https://downloads.php.net/~windows/releases/archives/php.zip".into();
    assert!(serverbond_core::install::archive_fallback(&php).is_none());
    let mysql = catalog().remove(1);
    assert!(serverbond_core::install::archive_fallback(&mysql).is_none());
}

fn cached_fixture(home: &std::path::Path, entry: &str) -> serverbond_core::model::Package {
...
fn repair_replaces_only_programs_and_keeps_old_files() {
    ...
    serverbond_core::install::repair(home.path(), &package, |_| {}).unwrap();
    serverbond_core::install::validate_installation(&dir, &package).unwrap();
...
fn failed_repair_keeps_existing_installation_intact() {
    ...
    assert!(serverbond_core::install::repair(home.path(), &package, |_| {}).is_err());

...
| Veritabanları | MySQL yanında PostgreSQL vb. paketler ve sürümler eklenebilir. [Quick-add](https://laragon.org/docs/quick-add) | MySQL 8.4 LTS yönetilir. Port, InnoDB belleği, bağlantı/paket limitleri, zaman aşımı, SQL modu, utf8mb4 karşılaştırması ve yavaş sorgu kaydı ayarlanır. İsteğe bağlı PostgreSQL 17 (`tools.json`, port 15432, Hizmetler → PostgreSQL) ortamı bloke etmez; `.env` yazılmaz. MySQL ana sürüm geçişi yok. |

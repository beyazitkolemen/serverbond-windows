...
Adlandırılmış MySQL işlemleri: `databases.show` sistem şemalarını gizleyip yerel `127.0.0.1` hesaplarının veritabanı yetkilerini listeler. `databases.create` doğrulanmış bir adı kullanarak yeni veritabanı açar. `databases.user` 1–20 mevcut veritabanı, yeni kullanıcı adı, 8–128 karakterli parola, salt okunur seçimi ve açık onay alır. Seçilen veritabanları dışında yetki verilmez; adlardaki alt çizgi `GRANT` içinde kaçırılır. Parola komut satırına, sonuca, günlüğe veya proje `.env` dosyasına yazılmaz. Cloud bu işlemleri cihaz tarafından bildirilen yetenek listesine göre açar.

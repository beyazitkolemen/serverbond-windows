...
Cihaz seviyesinde `mysql.connection` parametresiz olarak Windows loopback adresi, port ve root kullanıcı adını verir. `mysql.credentials` açık `confirm: true` ile mevcut parolayı alır. `mysql.password`, `password` ve `confirm: true` ile aynı Manager parola değiştirme akışını kullanır; sonuç yalnızca `changed: true` içerir. Parola 8–128 ASCII karakterdir; boşluk, tırnak, #, ; ve ters eğik çizgi kabul edilmez. Cloud tekrar alanını doğrular ve Windows'a göndermez. Parola parametre/sonuçları şifreli saklanır; Reverb yalnızca bildirim taşır. Panelde alınan parola iki dakikalık pencere içinde gösterilir ve yeni değişiklik isteğinde gizlenir; bu görsel süre kısıtı şifreli işlem kaydını silmez. PostgreSQL bağlantı/parola işlemleri henüz Cloud'a bağlı değildir.
